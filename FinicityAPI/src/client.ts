  ...
  constructor(config?: Partial<Configuration>) {
    ...
    this._requestBuilderFactory = createRequestHandlerFactory(
      ...
      [
        ...
        withFinicityAppKey(this._config),
      ],
...
function withErrorHandlers(rb: SdkRequestBuilder) {
  rb.defaultToError(ExpiredTokenError);
}
...
function withFinicityAppKey({ finicityAppKey }: { finicityAppKey: string }) {
  return (rb: SdkRequestBuilder) => {
    rb.interceptRequest(request => {
      const headers = request.headers ?? {};
      setHeader(headers, 'Finicity-App-Key', finicityAppKey);
